...
async function chatFetch(path: string, init: RequestInit, accessToken: string): Promise<Response> {
  ...
  const next = await tryRefreshCoordinatorSession();
  if (!next) return res;
...
export async function getGlobalChatRoom(): Promise<ChatRoomRow> {
  const rooms = await listChatRooms();
  const global = rooms.find((r) => r.type === "GLOBAL");
  if (!global) throw new Error("المحادثة العامة غير متاحة");
  return global;
}
